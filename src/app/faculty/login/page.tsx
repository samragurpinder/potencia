...
import { facultyData } from '@/components/FacultyList'
...
export default function FacultyLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const faculty = facultyData.find(f => f.id === formData.facultyId)
    
    if (faculty && formData.password === 'faculty123') {
      ...
      setError('Invalid credentials. Please try with your Faculty ID and password: faculty123')
    }
  }
    ...
    <main className="min-h-screen bg-black text-white flex items-center justify-center px-4">
      <div className="max-w-md w-full">
        ...
        <div className="bg-white/5 rounded-xl p-8 backdrop-blur-sm border border-white/10">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold mb-2">Faculty Login</h2>
            <p className="text-gray-400">Access your teaching dashboard</p>
          </div>
          ...
          <div className="mt-6 p-4 bg-white/5 rounded-lg">
            <p className="text-sm text-gray-400 text-center">
              Demo Credentials:
              <br />
              Faculty IDs: FACULTY001 to FACULTY005
              <br />
              Password: faculty123
            </p>
          </div>
